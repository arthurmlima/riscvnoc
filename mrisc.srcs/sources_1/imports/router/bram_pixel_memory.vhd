...
begin
    ...
    o_router_addr   <= (others => '0');

    -- AXI memory part
    o_axi_px        <= to_unsigned(to_integer(unsigned(s_doa)),o_axi_px'length);
    
    process(i_axi_en, i_axi_we,i_axi_x,i_axi_y,i_axi_s,i_axi_f,i_axi_px,s_doa)
        variable s_axi_addr : unsigned(31 downto 0);
    begin
        if(i_axi_en = '1') then
            s_axi_addr  :=  resize( img_size*(i_axi_x-x_init)*steps*frames+(i_axi_y-y_init)*steps*frames+
                                    i_axi_s*frames+i_axi_f, s_axi_addr'length);
            if(s_axi_addr < mem_size) then
                s_ena <= '1';
                s_addra     <= std_logic_vector(s_axi_addr(s_addra'length-1 downto 0));
                if(i_axi_we = '1') then
                    s_wea   <= '1';
                    s_dia   <= std_logic_vector(to_signed(to_integer(i_axi_px), s_dia'length));
                else
                    s_wea   <= '0';
                end if;
            end if;
    ...
    process(i_reset,i_wr_enable,i_enable,s_router_x,s_router_y,s_router_s,s_router_f,s_router_px,s_dob)
