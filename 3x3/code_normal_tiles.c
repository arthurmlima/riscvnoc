...
#define SUB_IMAG_WIDTH          (*(volatile uint32_t*)0x02310008)
#define SUB_IMAG_HEIGHT         (*(volatile uint32_t*)0x0231000C)
#define X_TILES                 (*(volatile uint32_t*)0x02310010)
#define Y_TILES                 (*(volatile uint32_t*)0x02310014)
#define X_LOCAL                 (*(volatile uint32_t*)0x02310018)
#define Y_LOCAL                 (*(volatile uint32_t*)0x0231001C)
#define X_INIT                  (*(volatile uint32_t*)0x02310020)
#define Y_INIT                  (*(volatile uint32_t*)0x02310024)
...
/**/
#define X_TILESi                     (*(volatile uint32_t*)0x00018424)
#define Y_TILESi                     (*(volatile uint32_t*)0x00018420)
#define X_LOCALi                     (*(volatile uint32_t*)0x0001841C)
#define Y_LOCALi                     (*(volatile uint32_t*)0x00018418)
#define SUB_IMAG_WIDTHi              (*(volatile uint32_t*)0x00018414)
#define SUB_IMAG_HEIGHTi             (*(volatile uint32_t*)0x00018410)
#define X_INITi                      (*(volatile uint32_t*)0x0001840C)
#define Y_INITi                      (*(volatile uint32_t*)0x00018408)
#define IMAGE_WIDTHi                 (*(volatile uint32_t*)0x00018404)
#define IMAGE_HEIGHTi                (*(volatile uint32_t*)0x00018400)
...

{
     ...
     IMAGE_WIDTHi=IMAGE_WIDTH;
     IMAGE_WIDTHi=IMAGE_WIDTH;
     Y_INITi=Y_INIT;
     X_INITi=X_INIT;
     Y_LOCALi=Y_LOCAL;
     X_LOCALi=X_LOCAL;
     SUB_IMAG_HEIGHTi=SUB_IMAG_HEIGHT;
     SUB_IMAG_WIDTHi=SUB_IMAG_WIDTH;
...
{
 volatile int pixel;
    for(volatile int i =Y_INITi ; i < SUB_IMAG_HEIGHTi+ Y_INITi; i++)
    {   
        for(volatile int j = X_INITi; j < SUB_IMAG_WIDTHi+ X_INITi; j++)
        {
        pixel=j+30*i;
        print_dec(pixel);
        print_dec(j);
...
void get_local_image(void)
{
 volatile int pixel;
    for(volatile int i = Y_INITi; i < SUB_IMAG_HEIGHTi+ Y_INITi; i++)
    {   
        for(int j = X_INITi; j < SUB_IMAG_WIDTH+ X_INITi; j++)
        {
...
void delay(void)
{
       for(volatile uint64_t ko = 0; ko < 20000; ko++);
}
...
print_dec(SUB_IMAG_WIDTH);
        print("\r\n\n\r");

print_dec(SUB_IMAG_HEIGHT);
        print("\r\n\n\r");
...
void distribute_image_from_zynq(void)
{
    ...
    print_dec(AXI_IMAGE_PIXEL);
    print("  "); 
    print_dec(AXI_IMAGE_X);
    print("  "); 
    print_dec(AXI_IMAGE_Y);
    
    // verificar se o pixel é da memoria interna 
    if(check_local_pixel(AXI_IMAGE_X,AXI_IMAGE_Y))
    {
        //interface com a PM
                // espera o req da pm ser 0
            
        set_pixel(AXI_IMAGE_PIXEL,AXI_IMAGE_X,AXI_IMAGE_Y,0,0);
                
            if(check_last_pixel(AXI_IMAGE_X,AXI_IMAGE_Y))
            {
                print("  ");
                print_dec(1);
               // write_gpio(0,X_INITi,Y_INIT,0,0,X_INITi,Y_INITi,1);
            }
    } 
     else 
    {
        //interface com o router 
        delay2(30000);
                        print("  ");
                print_dec(2);
...
int check_last_pixel(uint32_t x,uint32_t y)
{
    if( y== (Y_INITi+SUB_IMAG_HEIGHTi-1)){
        if( x== (X_INITi+SUB_IMAG_WIDTHi-1)){
            return 1;
        }
    }        
    return 0;
...
int check_local_pixel(uint32_t x,uint32_t y)
{
    if ((x >= X_INITi) & (x< X_INITi+SUB_IMAG_WIDTHi)){
      if((y >= Y_INITi) & (y< (Y_INITi+SUB_IMAG_HEIGHTi))){   
        return 1;
    }}
        return 0;
}
...
void send_transfer_finished_notice_recv(void)
{
    write_gpio(0,10,10,0,0,10,10,1);
}


void get_and_set_local_image_from_zynq_recv(void)
{
    for(volatile uint32_t i = 0; i < SUB_IMAG_HEIGHTi; i++)
    {   
        for(volatile uint32_t j = 0; j < SUB_IMAG_WIDTHi; j++)
        {
